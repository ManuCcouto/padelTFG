...
public class ChooseDayState  implements FormState {
    private FormBuilder formBuilder;

    private static final Logger logger = LoggerFactory.getLogger(ChooseDayState.class);

    @Override
    public void setMessage() {
        Update update = this.formBuilder.getUpdate();
        SendMessage message = new SendMessage();
        message.setChatId(this.getTelegramId(update));
        message.setText("Selecciona el día:");

        // Configurar botones de selección del día
        // ...
        InlineKeyboardMarkup dayButtons = Button.createDayButtons();
        message.setReplyMarkup(dayButtons);

        logger.info("startForm" + message.toString());
    ...
    public void processFormResponse(Update update) {
       ...
       // formBuilder.saveResponse("Día", response);
        ...
        message.setText("Por favor, selecciona un recurso:");
    ...
    private SendMessage echoMessage(Update update) {
        ...
        String responseText = "Has seleccionado: " + "HOY" ;
